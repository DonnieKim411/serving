...
Status PreProcessPrediction(const SignatureDef& signature,
  ...
  // sort inputs to make TF session not insert too many keys to executor @edit by donnie
  std::sort(
  ...
  std::sort(output_tensor_aliases->begin(), output_tensor_aliases->end()); // @edit by donnie
  std::sort(output_tensor_names->begin(), output_tensor_names->end());
