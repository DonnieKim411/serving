...
Status PreProcessPrediction(const SignatureDef& signature,
  ...
  // sort inputs to make TF session not insert too many keys to executor @donniekim
  std::sort(
  ...
  std::sort(output_tensor_aliases->begin(), output_tensor_aliases->end()); //added by @donniekim
